...
export class VoucherService {
  ...
  create(dto: CreateVoucherDTO): Voucher {
    ...
    if (period.status === 'closed') throw new Error('该期间已结账，无法新增凭证')
  ...
  private assertExists(id: string): Voucher {
    ...
    if (!v) throw new Error(`凭证 ${id} 不存在`)
    return v
  }
}
